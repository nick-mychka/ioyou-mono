...
> In this template, we use `@ioyou` as a placeholder for package names. As a user, you might want to replace it with your own organization or project name. You can use find-and-replace to change all the instances of `@ioyou` to something like `@my-company` or `@project-name`.
...
pnpm --filter @ioyou/auth generate
```
...
3. Outputs a Drizzle-compatible schema file to the `@ioyou/db` package
...
│                                                                     │
│  ┌──────────────────────┐         ┌──────────────────────┐         │
│  │   apps/tanstack-start │         │     apps/expo         │         │
│  │   (Web App)           │         │     (Mobile App)      │         │
│  │                       │         │                       │         │
│  │  React 19 + SSR       │         │  React Native 0.81    │         │
│  │  TanStack Router      │         │  Expo Router          │         │
│  │  Tailwind CSS v4      │         │  NativeWind v5        │         │
│  │  Port 3001            │         │  iOS / Android        │         │
│  └───────────┬───────────┘         └───────────┬───────────┘         │
│              │                                 │                     │
│              │  tRPC (local on server,         │  tRPC (HTTP)        │
│              │  HTTP on client)                │  + auth cookies     │
│              │                                 │                     │
│  ┌───────────┴─────────────────────────────────┴───────────┐        │
│  │                   Shared Packages                        │        │
│  │                                                          │        │
│  │  @ioyou/api          tRPC routers + procedures           │        │
│  │  @ioyou/auth         Better Auth (email/pw, Google, Apple)│       │
│  │  @ioyou/db           Drizzle ORM + schema                │        │
│  │  @ioyou/validators   Shared Zod schemas                  │        │
│  │  @ioyou/ui           shadcn/ui components (web only)     │        │
│  └──────────────────────────┬───────────────────────────────┘        │
│                             │                                        │
│                             │  Drizzle ORM                           │
│                             ▼                                        │
...
### Web App (apps/tanstack-start)

This is a **full-stack app** — the same process serves both the UI and the API.
...
│                                                  │
│  ┌──────────────────────────────────────┐       │
│  │          Page Routes (SSR + client)   │       │
│  │                                       │       │
│  │  /  →  index.tsx (your pages)         │       │
│  │  ...                                  │       │
│  │                                       │       │
│  │  On server: tRPC calls go directly    │       │
│  │  to the router (no HTTP roundtrip)    │       │
│  │                                       │       │
│  │  On client: tRPC calls go via         │       │
│  │  HTTP to /api/trpc                    │       │
│  └──────────────────────────────────────┘       │
└─────────────────────────────────────────────────┘
```
...
- `src/routes/` — add pages here (file-based routing)
- `src/component/` — shared React components
- `src/auth/client.ts` — call `authClient.signIn.email()`, `authClient.signIn.social()`, etc.

### Mobile App (apps/expo)

This is a **client-only app** — it talks to the web app's API over HTTP.

```
┌────────────────────────────┐       HTTP        ┌──────────────────┐
│        Expo App             │  ──────────────▶  │  Nitro Server    │
│                             │                   │  (port 3001)     │
│  Expo Router (file-based)   │  tRPC calls       │                  │
│  NativeWind for styling     │  ──────────────▶  │  /api/trpc/$     │
│  SecureStore for tokens     │                   │                  │
│                             │  Auth requests    │                  │
│  Detects host IP auto-      │  ──────────────▶  │  /api/auth/$     │
│  matically in dev           │                   │                  │
└────────────────────────────┘                   └──────────────────┘
```

**Key files you work with:**
- `src/app/` — add screens here (file-based routing)
- `src/utils/auth.ts` — same auth API as web, plus SecureStore for native token persistence
- `src/utils/api.tsx` — tRPC client (auto-attaches auth cookies to every request)

### Backend (shared packages)

You never run the backend separately — it lives inside the TanStack Start server. Both apps consume it through the same tRPC API.

```
                    ┌─────────────────────────────────────┐
                    │            @ioyou/api                │
                    │                                      │
                    │  root.ts  ← registers all routers    │
                    │    ├─ router/auth.ts  (getSession)    │
                    │    ├─ router/post.ts  (demo CRUD)     │
                    │    └─ router/your-feature.ts (add here)│
                    │                                      │
                    │  trpc.ts  ← defines procedures:      │
          ...
          ▼                    ▼                     ▼
┌─────────────────┐ ┌─────────────────┐  ┌─────────────────┐
│   @ioyou/auth    │ │    @ioyou/db     │  │ @ioyou/validators│
│                  │ │                  │  │                  │
│  Email/password  │ │  schema.ts       │  │  Shared Zod      │
│  Google OAuth    │ │  (your tables)   │  │  schemas for     │
│  Apple OAuth     │ │                  │  │  frontend +      │
│                  │ │  auth-schema.ts   │  │  backend         │
│  Session mgmt    │ │  (auto-generated)│  │                  │
└─────────────────┘ └────────┬─────────┘  └─────────────────┘
                             │
                             ▼
                      ┌──────────────┐
...
### How to add a new feature (typical flow)
...
2. Add backend logic
   packages/api/src/router/foo.ts   ← create router with procedures
   packages/api/src/root.ts         ← register the router

3. Build the web UI
   apps/tanstack-start/src/routes/foo.tsx
   └─ useSuspenseQuery(trpc.foo.list.queryOptions())
   └─ useMutation(trpc.foo.create.mutationOptions(...))

4. Build the mobile UI
   apps/expo/src/app/foo.tsx
   └─ useQuery(trpc.foo.list.queryOptions())
   └─ useMutation(trpc.foo.create.mutationOptions(...))
```

The tRPC types flow end-to-end: change a procedure's input/output in `@ioyou/api` and TypeScript immediately shows errors in both apps where the usage doesn't match.
...
Client                          Server (/api/auth)
  ...
  │  ◀──────────────────────────   │  returns session cookie
  │                                │
  │  tRPC call (cookie attached)   │
  │  ──────────────────────────▶   │  protectedProcedure reads session
  │                                │  from cookie → ctx.session.user
```

**OAuth (Google/Apple):**
```
Client                    Server (/api/auth)           Google/Apple
  │                          │                            │
  │  signIn.social()         │                            │
  │  ────────────────────▶   │                            │
...
**On mobile**, the `oAuthProxy` plugin handles the redirect through the web app's stable URL, then deep-links back to the Expo app via the `ioyou://` scheme.
